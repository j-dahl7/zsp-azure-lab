...
# Admission rejects a role the gateway holds no definition ID for, and only the
# keys of that map are read, so the stub carries the deployed role names alone.
nhi_module.ROLE_DEFINITIONS = dict.fromkeys(
    (
...
OTHER_GROUP_ID = "77777777-7777-4777-8777-777777777777"
OTHER_SP_ID = "88888888-8888-4888-8888-888888888888"
OTHER_SCOPE = (
    f"/subscriptions/{SUBSCRIPTION_ID}/resourceGroups/rg-elsewhere/"
    "providers/Microsoft.KeyVault/vaults/production"
)
...
class EndpointAllowlistTests(unittest.IsolatedAsyncioTestCase):
    ...
    happens to export cannot decide the outcome, and so an allowlist left out of a
    case is genuinely unset rather than inherited.
    """

    def _admin_body(self, **overrides):
        body = {
            "user_id": USER_ID,
            "group_id": GROUP_ID,
            "duration_minutes": 60,
    ...
    async def _admin_request(self, environment, body):
        client = FakeDurableClient()
        with patch.dict(os.environ, environment, clear=True):
            response = await function_app.admin_access_request(FakeRequest(body), client)
        return response, client

    async def _nhi_request(self, environment, body):
        client = FakeDurableClient()
        with patch.dict(os.environ, environment, clear=True):
            response = await function_app.nhi_access_request(FakeRequest(body), client)
        return response, client

    def _assert_refused(self, response, client, status_code, message):
        self.assertEqual(response.status_code, status_code)
        self.assertIn(message, json.loads(response.body)["error"])
        self.assertEqual(client.calls, [])

    async def test_admin_denies_a_group_outside_the_allowlist(self):
        response, client = await self._admin_request(
            {
                "ALLOWED_ADMIN_GROUP_IDS": OTHER_GROUP_ID,
                "ALLOWED_ADMIN_USER_IDS": USER_ID,
            },
            self._admin_body(),
        )

        self._assert_refused(response, client, 403, "Group is not allowed")

    async def test_admin_denies_a_user_outside_the_allowlist(self):
        response, client = await self._admin_request(
            {
                "ALLOWED_ADMIN_GROUP_IDS": GROUP_ID,
                "ALLOWED_ADMIN_USER_IDS": OTHER_USER_ID,
            },
            self._admin_body(),
        )

        self._assert_refused(response, client, 403, "User is not allowed")

    async def test_admin_refuses_when_the_group_allowlist_is_unset(self):
        response, client = await self._admin_request(
            {"ALLOWED_ADMIN_USER_IDS": USER_ID},
            self._admin_body(),
        )

        self._assert_refused(response, client, 503, "Admin group allowlist is not configured")

    async def test_admin_refuses_when_the_user_allowlist_is_unset(self):
        response, client = await self._admin_request(
            {"ALLOWED_ADMIN_GROUP_IDS": GROUP_ID},
            self._admin_body(),
        )

        self._assert_refused(response, client, 503, "Admin user allowlist is not configured")

    async def test_nhi_denies_a_service_principal_outside_the_allowlist(self):
        response, client = await self._nhi_request(
            {
                "ALLOWED_NHI_SP_OBJECT_IDS": OTHER_SP_ID,
                "ALLOWED_SCOPE_IDS": SCOPE,
            },
            self._nhi_body(),
        )

        self._assert_refused(response, client, 403, "Service principal is not allowed")

    async def test_nhi_denies_a_workflow_outside_the_allowlist(self):
        response, client = await self._nhi_request(
            {
                "ALLOWED_NHI_SP_OBJECT_IDS": SP_ID,
                "ALLOWED_WORKFLOW_IDS": "nightly-backup",
                "ALLOWED_SCOPE_IDS": SCOPE,
            },
            self._nhi_body(workflow_id="manual-test"),
        )

        self._assert_refused(response, client, 403, "Workflow ID is not allowed")

    async def test_nhi_denies_a_role_outside_the_allowlist(self):
        response, client = await self._nhi_request(
            {
                "ALLOWED_NHI_SP_OBJECT_IDS": SP_ID,
                "ALLOWED_NHI_ROLES": "Reader",
                "ALLOWED_SCOPE_IDS": SCOPE,
            },
            self._nhi_body(),
        )

        self._assert_refused(response, client, 403, "Role is not allowed")

    async def test_nhi_denies_a_scope_outside_the_allowlist(self):
        response, client = await self._nhi_request(
            {
                "ALLOWED_NHI_SP_OBJECT_IDS": SP_ID,
                "ALLOWED_SCOPE_IDS": OTHER_SCOPE,
            },
            self._nhi_body(),
        )
    ...
    async def test_nhi_refuses_when_the_service_principal_allowlist_is_unset(self):
        ...
        self._assert_refused(
            response, client, 503, "Service principal allowlist is not configured"
        )

    async def test_nhi_rejects_an_allowlisted_role_the_gateway_cannot_assign(self):
        # An operator can put any string in ALLOWED_NHI_ROLES. Admission has to
        # catch a role with no definition ID before the caller is told 202 and the
        # preflight fails on a missing map entry.
        response, client = await self._nhi_request(
            {
                "ALLOWED_NHI_SP_OBJECT_IDS": SP_ID,
                "ALLOWED_NHI_ROLES": "Owner",
                "ALLOWED_SCOPE_IDS": SCOPE,
            },
            self._nhi_body(role="Owner"),
        )

        self._assert_refused(response, client, 400, "Role is not supported by this gateway")
    ...
    def test_object_id_comparison_casefolds_both_sides(self):
        self.assertTrue(function_app._object_id_is_allowed(USER_ID, {USER_ID.upper()}))
        self.assertTrue(function_app._object_id_is_allowed(USER_ID.upper(), {USER_ID}))
        self.assertFalse(function_app._object_id_is_allowed(USER_ID, {OTHER_USER_ID}))
